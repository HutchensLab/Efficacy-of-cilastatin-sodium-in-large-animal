...
cil_calcil_veh <- c('#631879ff','#88CCEE',"red3") 
sham_cil_veh <- c(bluedarkcyan,"mediumblue","red3") 
cil_calcil <- c('#631879ff','#88CCEE') 
...
scalefillCC <-  scale_fill_manual(values = c(cil_calcil))
scalecolorCC <-  scale_color_manual(values = c(cil_calcil))
...
setmaxY<-function(maxdata, round=TRUE){
  if (maxdata<1){return(maxdata/10*11)}
  if(round){
...
results_flextable<-function(ps_data){
  ft<-flextable(ps_data)
  ft<-colformat_double(ft,digits=2)
  ft<-colformat_double(ft,j="x",digits=0)
  ft<-labelizor(ft,labels=c(x="time (h)"))
  ft<-autofit(ft)
  ft<-theme_vanilla(ft)
