...
title<-paste("Analysis underlying supplemental figures from ",b)
authors<-paste("By: ",c)
...
## Cilastatin activity assay results

```{r activity,warning=FALSE,  message=FALSE}
ylab<-"DPEP1 activity relative to control"
title<-"cilastatin_inhibitory_activity.tiff"
df<-read_xlsx(here(data,"Cilastatin_DPEP_assay.June2023.tidy.xlsx"))
t<-c("mce_bulk","trc_bulk","DPEP1_control","sigma_c5743","sigma_sml1283")
df<-df%>%filter(treatment%in%t)
df<-df%>%mutate(control_mean=mean(FI[treatment=="DPEP1_control"]))%>%
  mutate(FIrc=FI/control_mean)
df<-df%>%mutate(tx_label=case_when(
  treatment=="DPEP1_control"~"Control",
  treatment=="mce_bulk"~"Bulk supplier 1",
  treatment=="trc_bulk"~"Bulk supplier 2",
  treatment=="sigma_c5743"~"Comparator 1",
  treatment=="sigma_sml1283"~"Comparator 2",
))%>%
  mutate(tx_label=fct_relevel(tx_label,"Control"))

droplevels(df)

ggplot(df,aes(x=tx_label, y=FIrc))+geom_point()+theme_classic()+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
#analysis
#statistical analysis - lmer followed by bh-adjusted pairwise
model<-aov(FIrc~tx_label, data=df)
summary(model)     
TukeyHSD(model)
emm_model <- emmeans(model,~tx_label,level=0.95)

custom <- list("Control vs bulk 1"= c(1,-1,0,0,0),
              "Control vs bulk 2"= c(1,0,-1,0,0),
               "Control vs comparator 1"= c(1,0,0,-1,0),
              "Control vs comparator 2"= c(1,0,0,0,-1),
              "bulk 1 vs bulk 2"= c(0,1,-1,0,0),
              "bulk vs comparators"=c(0,1,1,-1,-1)
              )

e<-emmeans(model,~tx_label, data=df, adjust = "sidak")
contrast(e,custom)
q<-column_scatter_errorbar_plot(df,x=tx_label,y=FIrc, groupvar=tx_label,themevar=plot_theme_squareish2,
                                scalecolor=scalecolorcilact,
                                scalefill=scalefillcilact,                                
                                p_brackets=FALSE,
...
q<-q+geom_signif(xmin=1,
                 ...
                 color="black",
                 vjust=-1)+
  geom_segment(x=2,xend=5,y=1.46,color="black", linewidth = 0.3)

q
#write the plot
if(WRITE_PLOTS){ggsave(filename=here(figure_parts,title),plot=q, width=5.0, height=5,dpi=300,units="in")}
...
```{r uMB,warning=FALSE,  message=FALSE}
ylab<-"Urine Mb (ng)"
title<-"urMb_cil_veh.tiff"

mb<-M%>%select(x,urMB,plMB,x_units,uop_rate,weight_kg,animal)%>%
  mutate(y=(urMB*uop_rate), units_x=x_units,weight=weight_kg)
mb<-mb%>%
...
p<-column_scatter_errorbar_plot(mb,x=factor(x),y=y, groupvar=tx,themevar=plot_theme_wide,
                                ...
                                p_brackets=TRUE,p_bracketsdata=brackets_table,bracket_nudge=-50,
...
```{r uPOR,warning=FALSE,  message=FALSE}
ylab<-"Urine Porphyrins (ug)"
title<-"urPOR_cil_veh.tiff"

up<-M%>%select(x,urPOR,x_units,uop_rate,weight_kg,animal)%>%
  mutate(y=(urPOR*uop_rate*1000), units_x=x_units,weight=weight_kg)
up<-up%>%
  filter(x%in%sched_labs_x)%>%
  mutate(tx=factor(case_when(
    animal%in%cila_tx ~ "cil",
...
#statistical analysis - lmer followed by adjusted pairwise
model<-lmer(y~tx*as.factor(x)+(1|animal),data=up)
summary(model)       
emm_model <- emmeans(model,~tx*as.factor(x),level=0.95)
emm_means<-print(emm_model)
pairs_model<-pairs(emm_model,by = "x", adjust="sidak")
...
maxy<-setmaxY(max(up$y))
brackets_table<-pairs_to_significance_table(pair_summary,maxy)


p<-column_scatter_errorbar_plot(up,x=factor(x),y=y, groupvar=tx,themevar=plot_theme_wide,
                                scalecolor=scalecolorCV,
                                scalefill=scalefillCV,
                                p_brackets=TRUE,p_bracketsdata=brackets_table,bracket_nudge=-25,
                                xlabel="Time (h)",ylabel=ylab)
#thisplot_left_legend<-c(0.35,0.95)
#p<-p+theme(legend.position = thisplot_left_legend)
p
...
#summary statistics for significant values formatted to place in the text of the paper
emm_sig_summary_paper(up,pair_summary,emm_means)
...
cr_d2448<-cr%>%group_by(animal)%>%
  filter(animal!=15&animal!=29&animal!=57)%>%#&animal!=77&animal!=82)%>% #remove animals which do not have a 48h gfr
  mutate(cr48=cr[x==48], 
         cr24=cr[x==24],
