...
model<-lme4::lmer(y~tx*as.factor(x)+(1|animal),data=gfr)

#reviewer 3 suggested we check for assumption violation/nonnormality
#robustness of lmer to violations of assumption (residual distribution primarily)
#is somewhat controversial.  See doi 10.1111/2041-210X.13434 and 10.18637/jss.v075.i06
#we do this by residual plot
resid_panel(model)
meets_assumptions=TRUE 

if(meets_assumptions==FALSE){
  #assumptions violated, therefore use rmodel for more outlier-robust lmer
...
#analysis
#statistical analysis - lmer followed by bh-adjusted pairwise
model<-lme4::lmer(y~tx*as.factor(x)+(1|daylen),data=cr)
...
#analysis
#statistical analysis - lmer followed by bh-adjusted pairwise
model<-lme4::lmer(y~tx*as.factor(x)+(1|daylen),data=bun)
...
if(meets_assumptions==FALSE){
  #non-normal, therefore use rmodel for outlier-robust lmer
    model<-rlmer(y~tx*as.factor(x)+(1|daylen),data=bun)
}
...
model<-lme4::lmer(value~tx*timepoint+(1|animal), data=ddat2448)
summary(model)
...
## K interventions Survival Analysis

```{r kintsurv ,warning=FALSE,  message=FALSE}

library (survival)
library(survminer)
source(here(code,"plot_common.R"))

title<-"KIV_KM_cil_veh2_notable.tif"
...
#analysis
#statistical analysis - lmer followed by bh-adjusted pairwise
model<-lme4::lmer(y~tx*as.factor(x)+(1|animal),data=kivnc)
...
##  time to recovery of creatinine (time to event)
``` {r time to creat recovery, warning=FALSE, message=FALSE}
title<-"time_to_creatinine_recovery.tif"
cr<-M%>%select(x,plCR,x_units,plCR_units,weight_kg,animal,daylen)%>%
  mutate(cr=plCR,y=plCR, units_x=x_units, units_y=plCR_units,weight=weight_kg)
cr<-cr%>%
  filter(x%in%sched_labs_x)%>%
